...
def load_data(data_dir, train_transform, valid_transform):
    ...
    valid_dataloader = DataLoader(valid_dataset, batch_size=70)
    test_dataloader = DataLoader(test_dataset, batch_size=70)
...
def train_model(model, trainloader, validloader, criterion, learning_rate, epochs, device, print_every=40):
    steps = 0
...
def main():
    ...
    train_model(model, trainloader, validloader, criterion, learning_rate, args.epochs, device)

    save_checkpoint(model, trainloader.dataset, args.save_dir ,args.arch)

    load_model('checkpoint.pth',args.arch)
